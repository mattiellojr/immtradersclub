...
class ProfitshareincomeController  extends MY_Controller
{
	...
	public function index () {
				...
				if( $this->is_logged_in() ) {
					...
					$this->load->library("pagination");
						...
						$config 					= array();
        				$config["base_url"] 		= base_url() . "bonus-reports/profit-share-income";
        				$config["total_rows"] 		= $this->model_transactionhistory->count_ref(['user_id' => $userid,'type_id' => 5]);
				        $config["per_page"] 		= 10;
				       ...
				       $page = 0 ;
						...
						$data = [
								...
								'results'			=> $this->model_transactionhistory->fetch_record( $config["per_page"], $page , [ 'transactionhistory.user_id' => $userid ,'transactionhistory.type_id' => 5] ,'transactiontype','transactionhistory.type_id=transactiontype.type_id'),
					...
					return $this->load->view('bonusreport/profitshareincome',$data);
				}
				else {
					redirect('login');
				}
	...
	public function get_history() {
		...
		if( $this->is_logged_in() ) {
			...
			$data   =   [] ;
				...
				foreach ($this->model_transactionhistory->query("SELECT transactionhistory.*,users.username  FROM transactionhistory JOIN users ON transactionhistory.sender_id=users.user_id WHERE  transactionhistory.user_id='".$userid."' AND  transactionhistory.type_id='5' ORDER BY transaction_id desc LIMIT 1000 ")->result() as $key => $value) {
